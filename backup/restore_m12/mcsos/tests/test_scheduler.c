...
int main(void) {
    ...
    REQUIRE(sched.runnable_count == 2u);
    REQUIRE(mcsos_sched_validate(&sched) == MCSOS_SCHED_OK);
    ...
    REQUIRE(a.ticks == 1u);
    REQUIRE(sched.ticks == 1u);

    REQUIRE(mcsos_sched_yield(&sched) == MCSOS_SCHED_OK);
    REQUIRE(sched.current == &b);
    REQUIRE(b.state == MCSOS_THREAD_RUNNING);
    REQUIRE(mcsos_sched_validate(&sched) == MCSOS_SCHED_OK);
    ...
    REQUIRE(mcsos_thread_block_current(&sched) == MCSOS_SCHED_OK);
    REQUIRE(sched.current == &b);
    REQUIRE(a.state == MCSOS_THREAD_BLOCKED);
    REQUIRE(mcsos_thread_mark_ready(&sched, &a) == MCSOS_SCHED_OK);
    REQUIRE(a.state == MCSOS_THREAD_READY);
    REQUIRE(mcsos_sched_validate(&sched) == MCSOS_SCHED_OK);
